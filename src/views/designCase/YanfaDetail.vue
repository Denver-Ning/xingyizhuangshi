  ...
  <div class="caseDetail">
    ...
    <div class="caseInfo">
      ...
      <div class="infoContent">
        <div>案例名称:{{ info.title }}</div>
        <div>案例风格:{{ info.design_style_name }}</div>
        <div>
          面积户室:{{ info.area }}㎡
          <span class="ml-2">{{ info.house_type_name }}</span>
        </div>
        <div>装修用材:1地砖、墙纸等</div>
        <div>工程造价:{{ info.price }}</div>
      </div>
      <!-- 设计师 -->
...
export default {
  ...
  methods: {
    async yanfaDetailInfo() {
      await this.$http
        .post("http://xyzs.demo365day.cn/index.php/api/team/houseinfo.html", {
          id: this.id,
        })
        .then(({ data }) => {
          this.$emit("anli", data.info.name);
          if (
