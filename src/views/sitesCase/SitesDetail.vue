<template>
  <div class="caseDetail">
    <div class="case">
      ...
      <div class="videoInfo" v-if="video">
        ...
        <div class="video" id="video">
          <video controls>
            <source :src="info.video_file" type="video/mp4" />
          </video>
        </div>
        <div class="videoContent" v-html="info.video_des"></div>
      </div>
      <!-- 详情介绍 -->
      <div class="detailInfo">
        ...
        <div class="detailImg">
          <img src="@/assets/image/shejituandui/anli1.png" alt="" />
        </div>
    ...
    <!-- 右侧案例信息 -->
    <div class="caseInfo">
      <div class="infoTitle">工地信息</div>
      <div class="infoContent">
        <div>案例名称:{{ info.name }}</div>
        <div>案例风格:{{ info.design_style }}</div>
        <div>
          面积户室:{{ info.area }}㎡
          <span class="ml-2">{{ info.house_type }}</span>
        </div>
        <div>工程造价:70万</div>
        <div>项目经理:{{ info.case_workers }}</div>
        <div>工程总监:{{ info.top_workers }}</div>
        <div>施工阶段:{{ info.build_stage }}</div>
      </div>
      <!-- 设计师 -->
...
export default {
  ...
  methods: {
    async getSitesDetail() {
      await this.$http
        .post("http://xyzs.demo365day.cn/index.php/api/team/buildinfo.html", {
          id: this.id,
        })
        .then(({ data }) => {
          this.$emit("gongdiTitle", data.info.name);
          if (
            data.info.video_file == "" ||
            data.info.video_file == undefined ||
            data.info.video_file == null
          ) {
            this.video = false;
          } else {
            this.video = true;
            $("#video").html(
              `<video style="width:100%" controls><source src="${data.info.video_file}" type="video/mp4"></source></video>`
            );
          }
          this.sitesDetailList = data;
    ...
    async yuyue() {
      if (this.userForm.name == "" || this.userForm.phone == "") {
        ...
        if (!sj.test(this.userForm.phone) && !zj.test(this.userForm.phone)) {
