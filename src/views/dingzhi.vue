<template>
  <div class="container-fuild marketing">
    <div class="banner">
      <img :src="dingzhiBanner.image" alt="" />
      <div class="content">
        <div class="line"></div>
        <div class="title">{{ dingzhiBanner.title }}</div>
        <div>
          {{ dingzhiBanner.des }}
        </div>
      </div>
    ...
    <div class="current">
      <div class="center">
        <span @click="toHome">首页</span>> <span>定制</span>
      </div>
      <div class="bg"></div>
    </div>
  ...
  methods: {
    ...
    async getDingzhiInfo() {
      await this.$http
        ...
        .then(({ data }) => {
          this.dingzhiBanner = data.banner;
        });
    },
  },
  mounted() {
